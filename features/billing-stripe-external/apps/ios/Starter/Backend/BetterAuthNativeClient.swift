...
actor BetterAuthNativeClient {
  ...
  func billingPortalURL(using bearerToken: BetterAuthBearerToken) async throws -> URL {
    let (data, _) = try await request(
      path: "/api/auth/subscription/billing-portal",
      method: "POST",
      json: [
        "returnUrl": "starter://billing/return",
        "disableRedirect": true,
      ],
      bearerToken: bearerToken
    )
...
final class BetterAuthProvider: AuthProvider, @unchecked Sendable {
  ...
  func billingPortalURL() async throws -> URL {
    ...
    return try await authClient.billingPortalURL(using: bearerToken)
  }
