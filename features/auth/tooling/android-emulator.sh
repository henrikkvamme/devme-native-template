...
readonly auth_config_file="${AUTH_CONFIG_FILE:-$root/.env.auth.local}"
...
if [[ -z "$google_web_client_id" && -f "$auth_config_file" && -f "$root/tooling/auth-config.ts" ]]; then
  google_web_client_id="$(
    bun "$root/tooling/auth-config.ts" android-client-id --input "$auth_config_file"
  )"
fi
