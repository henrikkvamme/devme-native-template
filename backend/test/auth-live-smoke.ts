...
await expectStatus(subscriptionsResponse, 200);
const subscriptions = (await subscriptionsResponse.json()) as unknown;
if (!Array.isArray(subscriptions)) {
  throw new Error("Stripe subscription endpoint returned an invalid payload");
}

const unsignedWebhookResponse = await fetch(`${authSiteURL}/api/auth/stripe/webhook`, {
  method: "POST",
  body: "{}",
});
await expectStatus(unsignedWebhookResponse, 400);
