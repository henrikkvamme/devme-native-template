...
2. Configure every Release build setting currently containing `replace-before-release`, `replace-with`, or `dev.starter.app`. Backend and auth endpoints must use HTTPS.
3. Create the matching app record in App Store Connect.
4. Verify every capability and identifier in Certificates, Identifiers & Profiles. This branch requires Sign in with Apple.
